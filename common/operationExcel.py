...
class OperationExcel:
    ...
    # 将返回的状态码和断言结果写入Excel
    def write_actual_return(self, status_code, assert_result, row):
        rb = self.book
        w_b = copy(rb)
        w_sheet = w_b.get_sheet(self.num)
        w_sheet.write(row, TestCaseKeyWord.actual_status_code, status_code)
        w_sheet.write(row, TestCaseKeyWord.result, assert_result)
        w_b.save(filepath(self.fileroot, self.excelname))
